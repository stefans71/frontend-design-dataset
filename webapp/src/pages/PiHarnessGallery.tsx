...
export default function PiHarnessGallery() {
  ...
  useEffect(() => {
    sessionStorage.setItem('pi-harness-gallery-page', String(page))
  }, [page])
            ...
            <form
              onSubmit={e => {
                ...
                if (numMatch) {
                  ...
                  navigate(`/pi-harness/components/component-${num}-run${run}`)
                }
              }}
            ...
            <>
              <div key={page} className={`grid ${gridClass} gallery-grid`} style={{ gap: 16 }}>
                {items.map((c, i) => (
                  ...
                  >
                    <ComponentCard component={c} index={i} basePath="/pi-harness/components" />
                  </div>
                ))}
              </div>
